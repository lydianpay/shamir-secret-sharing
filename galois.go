...
// gf represents bytes 0 - 255 in a Galois Field (2^8)
var gf = [256]byte{
...
// precomputed represents precomputed bytes 0 - 255 for multiplication and division in a Galois Field (2^8)
var precomputed = [256]byte{
...
func multiplyGF(a, b uint8) (out uint8) {

	if a == 0 || b == 0 {
		return 0
	}

	mod := (int(gf[a]) + int(gf[b])) % 255

	return precomputed[mod]
...
func divideGF(a, b uint8) (uint8, error) {
	...
	mod := (int(gf[a]) - int(gf[b]) + 255) % 255

	return precomputed[mod], nil
}
